<?php
/**
 * Tiny File Manager – protected JSON API
 *
 * This endpoint is intentionally independent from the browser UI.
 * Configure access in config.php using:
 *   $api_enabled = true;
 *   $api_tokens = array('token' => array('role' => 'admin', 'root_path' => __DIR__ . '/Mirko'));
 *
 * Authentication:
 *   Authorization: Bearer <token>
 *   or X-TFM-API-Key: <token>
 */
...
switch ($action) {
    case 'ping':
        api_json_response(true, array(
            'message' => 'TinyFileManager API is available.',
            'root' => $api_root,
        ));
        break;
    ...
    case 'read':
        ...
        api_json_response(true, array(
            'path' => api_normalize_relative_path($path),
            'encoding' => 'utf-8',
            'content' => $content,
        ));
        break;
