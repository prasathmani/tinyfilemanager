    ...
    try{
        if(isset($fileManagerConfig)){
            ...
            if(version_compare($lastVersion, VERSION, '>')){
                ...
                if($fileManagerConfig['root_path'] == $_SERVER['DOCUMENT_ROOT']){
                    $tmp = str_replace("'root_path' => '".$_SERVER['DOCUMENT_ROOT']."',", "'root_path' => $"."_SERVER['DOCUMENT_ROOT'],", $tmp);
                }
                if($fileManagerConfig['http_host'] == $_SERVER['HTTP_HOST']){
                    $tmp = str_replace("'http_host' => '".$_SERVER['HTTP_HOST']."',", "'http_host' => $"."_SERVER['HTTP_HOST'],", $tmp);
                }

                $newFM = str_replace('$'.'fileManagerConfig = null;', $tmp, $newFM);
                file_put_contents(basename(__FILE__), $newFM);
                die();
            }
        }
...
// if fm included
if (defined('FM_EMBED')) {
    $fileManagerConfig['use_auth'] = false;
    $fileManagerConfig['sticky_navbar'] = false;
} else {
    @set_time_limit(600);

    date_default_timezone_set($fileManagerConfig['default_timezone']);
...
if (empty($fileManagerConfig['auth_users'])) {
    $fileManagerConfig['use_auth'] = false;
}
...
// update $fileManagerConfig['root_url'] based on user specific directories
if (isset($_SESSION[FM_SESSION_ID]['logged']) && !empty($fileManagerConfig['directories_users'][$_SESSION[FM_SESSION_ID]['logged']])) {
    $wd = fm_clean_path(dirname($_SERVER['PHP_SELF']));
    $fileManagerConfig['root_url'] =  $fileManagerConfig['root_url'].$wd.DIRECTORY_SEPARATOR.$fileManagerConfig['directories_users'][$_SESSION[FM_SESSION_ID]['logged']];
}
// clean $fileManagerConfig['root_url']
$fileManagerConfig['root_url'] = fm_clean_path($fileManagerConfig['root_url']);

// abs path for site
defined('FM_ROOT_URL') || define('FM_ROOT_URL', ($is_https ? 'https' : 'http') . '://' . $fileManagerConfig['http_host'] . (!empty($fileManagerConfig['root_url']) ? '/' . $fileManagerConfig['root_url'] : ''));
defined('FM_SELF_URL') || define('FM_SELF_URL', ($is_https ? 'https' : 'http') . '://' . $fileManagerConfig['http_host'] . $_SERVER['PHP_SELF']);
...
if($fileManagerConfig['ip_ruleset'] != 'OFF'){
    $clientIp = $_SERVER['REMOTE_ADDR'];

    $proceed = false;

    $whitelisted = in_array($clientIp, $fileManagerConfig['ip_whitelist']);
    $blacklisted = in_array($clientIp, $fileManagerConfig['ip_blacklist']);

    if($fileManagerConfig['ip_ruleset'] == 'AND'){
        if($whitelisted == true && $blacklisted == false){
            $proceed = true;
        }
    } else
    if($fileManagerConfig['ip_ruleset'] == 'OR'){
         if($whitelisted == true || $blacklisted == false){
            $proceed = true;
        }
    }
...
// Auth
if ($fileManagerConfig['use_auth']) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $fileManagerConfig['auth_users'][$_SESSION[FM_SESSION_ID]['logged']])) {
        // Logged
    } elseif (isset($_POST['fm_usr'], $_POST['fm_pwd'])) {
        // Logging In
        sleep(1);
        if(function_exists('password_verify')) {
            if (isset($fileManagerConfig['auth_users'][$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $fileManagerConfig['auth_users'][$_POST['fm_usr']])) {
...
// update root path
if ($fileManagerConfig['use_auth'] && isset($_SESSION[FM_SESSION_ID]['logged'])) {
    $fileManagerConfig['root_path'] = isset($fileManagerConfig['directories_users'][$_SESSION[FM_SESSION_ID]['logged']]) ? $fileManagerConfig['directories_users'][$_SESSION[FM_SESSION_ID]['logged']] : $fileManagerConfig['root_path'];
}

// clean and check $fileManagerConfig['root_path']
...
defined('FM_ROOT_PATH') || define('FM_ROOT_PATH', $fileManagerConfig['root_path']);
defined('FM_LANG') || define('FM_LANG', $lang);
defined('FM_FILE_EXTENSION') || define('FM_FILE_EXTENSION', $fileManagerConfig['allowed_file_extensions']);
defined('FM_UPLOAD_EXTENSION') || define('FM_UPLOAD_EXTENSION', $fileManagerConfig['allowed_upload_extensions']);
defined('FM_EXCLUDE_ITEMS') || define('FM_EXCLUDE_ITEMS', $fileManagerConfig['exclude_items']);
defined('FM_DOC_VIEWER') || define('FM_DOC_VIEWER', $fileManagerConfig['online_viewer']);
define('FM_READONLY', $fileManagerConfig['use_auth'] && !empty($readonly_users) && isset($_SESSION[FM_SESSION_ID]['logged']) && in_array($_SESSION[FM_SESSION_ID]['logged'], $readonly_users));
...
define('FM_PATH', $p);
define('FM_USE_AUTH', $fileManagerConfig['use_auth']);
define('FM_EDIT_FILE', $fileManagerConfig['edit_files']);
defined('FM_ICONV_INPUT_ENC') || define('FM_ICONV_INPUT_ENC', $fileManagerConfig['iconv_input_encoding']);
defined('FM_USE_HIGHLIGHTJS') || define('FM_USE_HIGHLIGHTJS', $fileManagerConfig['use_highlightjs']);
defined('FM_HIGHLIGHTJS_STYLE') || define('FM_HIGHLIGHTJS_STYLE', $fileManagerConfig['highlightjs_style']);
defined('FM_DATETIME_FORMAT') || define('FM_DATETIME_FORMAT', $fileManagerConfig['datetime_format']);

unset($p, $fileManagerConfig['use_auth'], $fileManagerConfig['iconv_input_encoding'], $fileManagerConfig['use_highlightjs'], $fileManagerConfig['highlightjs_style']);
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete')?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete folder?');"><i class="fa fa-trash-o" style="background:#FF4136;" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('Rename')?>" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" style="background:#FF851B" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('CopyTo')?>..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink')?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="fa fa-link" style="background:#001f3f" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
                flush();
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        <a title="<?php echo lng('Preview') ?>" href="<?php echo $filelink.'&quickView=1'; ?>" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win($f) ?>" data-max-width="100%" data-width="100%"><i class="fa fa-eye" style="background: #fff400;"></i></a>
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('File').'?'; ?>');"><i class="fa fa-trash-o" style="background:#FF4136;"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" style="background:#FF851B"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..."
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link" style="background:#001f3f"></i></a>
                        <a title="<?php echo lng('Download') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download" style="background:#2ECC40"></i></a>
                    </td>
                </tr>
                <?php
                flush();
 ...
 class FM_Config
{
    ...
    {
        global $fileManagerConfig, $CONFIG;
        $fm_url = $fileManagerConfig['root_url'].$_SERVER["PHP_SELF"];
        $this->data = array(
            'lang' => 'en',
    ...
    function save()
    {
        ...
        $fm_file = $fileManagerConfig['root_path'].$_SERVER["PHP_SELF"];
        $var_name = '$CONFIG';
...
{
    global $lang, $fileManagerConfig;
    $isStickyNavBar = $fileManagerConfig['sticky_navbar'] ? 'fixed-top' : '';
    ?>
    <nav class="navbar navbar-expand-lg navbar-light bg-white mb-4 main-nav <?php echo $isStickyNavBar ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = fm_clean_path($path);
            $fileManagerConfig['root_url'] = "<a href='?p='><i class='fa fa-home' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
            ...
            if ($path != '') {
                ...
                $fileManagerConfig['root_url'] .= $sep . implode($sep, $array);
            }
            echo '<div class="col-xs-6 col-sm-5">' . $fileManagerConfig['root_url'] . '</div>';
            ?>
...
global $lang, $fileManagerConfig;
$isStickyNavBar = $fileManagerConfig['sticky_navbar'] ? 'navbar-fixed' : 'navbar-normal';
?>
<!DOCTYPE html>
<html>
<head>
