...
# Usage:
#   ./prehliadac.sh
#   ./prehliadac.sh --port 8090
#   ./prehliadac.sh --no-open
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		-h|--help)
			cat <<EOF
Usage: ./prehliadac.sh [--port PORT] [--no-open]
...
if git -C "$WORKTREE_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
	...
	echo "[2/4] Path already exists but is not a git checkout: $WORKTREE_DIR" >&2
	echo "Remove or rename it first, or point the script to an empty path." >&2
	exit 1
else
