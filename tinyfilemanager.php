...
// Define libraries versions to be used
define('BOOTSTRAP_VERSION', '5.2.3');
define('BOOTSTRAP_INTEGRITY_JS', 'sha384-kenU1KFdBIe4zVF0s0G1M5b4hcpxyD9F7jL+jjXkk+Q2h455rYXK/7HAuoJl+0I4');
define('BOOTSTRAP_INTEGRITY_CSS', 'sha384-rbsA2VBKQhggwzxH7pPCaAqO46MgnOM80zW1RWuH61DGLwZJEdK2Kadq2F9CUG65');
define('JQUERY_VERSION', '3.6.3');
define('JQUERY_INTEGRITY_JS', 'sha256-pvPw+upLPUjgMXY0G+8O0xUf+/Im1MZjXxxgOcBQBXU=');
define('DATATABLES_VERSION', '1.10.25');
define('DATATABLES_INTEGRITY_JS', 'sha384-rfvKADMCRZpVdF4/h4VOowFi3EyKPCoFj7EisSOtOTwJzIA44wSTfQzSLbNPKW6F');
define('EKKO_LIGHTBOX_VERSION','5.3.0');
define('EKKO_LIGHTBOX_INTEGRITY_CSS','sha384-degZUV6f5Qw/AGD0Kq6kEC7j8V6h+IWPfm+AJ+2q3gaTFa9U4fu/9u3Egh7xOYE4');
define('EKKO_LIGHTBOX_INTEGRITY_JS','sha384-bqIl0+8hxcMpOFf8O3ffwAy5qTUbFPyuNFytX/omkD4RDPMOBtt/uV6Lqlw7sljM');
define('FONT_AWESOME_VERSION', '4.7.0');
define('FONT_AWESOME_INTEGRITY_CSS', 'sha384-wvfXpqpZZVQGK6TAh5PVlGOfQNHSoD2xbE+QkPxCAFlNEevoEH3Sl0sibVcOQVnN');
define('HIGHLIGHT_VERSION', '11.7.0');
define('HIGHLIGHT_INTEGRITY_JS', 'sha384-4l+9bhb7rakZ18megzl0/DWczL8ojbDl1jIEzBVffeMho9A6xB/lkqt1K0PC8Jin');
define('HIGHLIGHT_INTEGRITY_CSS', 'sha384-Ao8ogxjh8YOpD0QuD98ftM0JgunR9NBRo18vvW76DgMR3Ek/XScY5KdUdV1Ams5S');
define('DROPZONE_VERSION', '5.9.3');
define('DROPZONE_INTEGRITY_CSS', 'sha384-hKRH7ZmTc4+t+iae668SDRfEsjc7HT3VrEMKuSwiDUK4pNQXd/v9BPVpIa0OLlp7');
define('DROPZONE_INTEGRITY_JS', 'sha384-PwiT+fWTPpIySx6DrH1FKraKo+LvVpOClsjx0TSdMYTKi7BR1hR149f4VHLUUnfA');
define('ACE_VERSION', '1.13.1');
define('ACE_INTEGRITY_JS', 'sha384-DlXjIzGWfxLAJS41DbZEi8G7QR8nLC5GGICZTbEVprhIuEj1Ij3gir+eLc3o0jp4');
...
getusernames_etcpasswd();
getgroupnames_etcgroup();
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
        ...
        if(function_exists('password_verify')) {
            if (!verifyToken($_POST['token'])) {
                ...
                unset($_SESSION[FM_SESSION_ID]['username']);
                unset($_SESSION[FM_SESSION_ID]['is_admin']);
                audit_action('login', 'fm_usr='.$_POST['fm_usr'].',msg=invalid-username-or-password-token-invalid', 'error');
                fm_set_msg(lng('Login failed. Invalid username or password'), 'error');
                fm_redirect(FM_SELF_URL);
            } else if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']])) {
                ...
                fm_set_msg(lng('You are logged in') . ($is_admin ? ' (as admin)' : ''));
                $audit_username = $_SESSION[FM_SESSION_ID]['username'];
                        ...
                        <div class="card fat <?php echo fm_get_theme(); ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="mb-3">
                                       <div class="brand">
                                            <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="100%" height="80px" viewBox="0 0 238.000000 140.000000" aria-label="H3K Tiny File Manager">
...
if (isset($_POST['newfilename'], $_POST['newfile'], $_POST['token']) && !FM_READONLY) {
    ...
    if (fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.' && verifyToken($_POST['token'])) {
        $path = os_path_join(FM_ROOT_PATH, FM_PATH);
        $file_path = os_path_join($path, $new);
        if ($type == "file") {
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    // abs path from
    $from = os_path_join(FM_ROOT_PATH, $copy);
    // abs path to
    $dest = os_path_join(FM_ROOT_PATH, FM_PATH, basename($from));
    // move?
    $move = isset($_GET['move']);
    $move = fm_clean_path(urldecode($move));
    // copy/move/duplicate
    if ($from != $dest) {
        $msg_from = trim(os_path_join(FM_PATH, basename($from), '/'));
       ...
       if (!$move){ //Not move and to = from so duplicate
            ...
            $extension_suffix = '';
            if(!is_dir($from) && isset($fn_parts['extension'])){
               $extension_suffix = '.'.$fn_parts['extension'];
            }
            //Create new name for duplicate
            $fn_duplicate = os_path_join($fn_parts['dirname'], $fn_parts['filename'].'-'.date('YmdHis').$extension_suffix);
            $loop_count = 0;
            $max_loop = 1000;
            // Check if a file with the duplicate name already exists, if so, make new name (edge case...)
            while(file_exists($fn_duplicate) & $loop_count < $max_loop){
               $fn_parts = pathinfo($fn_duplicate);
               $fn_duplicate = os_path_join($fn_parts['dirname'], $fn_parts['filename'].'-copy'.$extension_suffix);
               $loop_count++;
            }
    ...
    if (is_array($files) && count($files)) {
        foreach ($files as $f) {
            if ($f != '') {
                $f = fm_clean_path($f);
                // abs path from
                $from = os_path_join($path, $f);
                // abs path to
                $dest = os_path_join($copy_to_path, $f);
                // do
                if ($move) {
                    $rename = fm_rename($from, $dest);
                    if ($rename === false) {
...
/*if (isset($_GET['dl'], $_POST['token'])) {
    ...
    $dl = str_replace('/', '', fm_clean_path($dl));
    $path = os_path_join(FM_ROOT_PATH, FM_PATH);
...
// Execute
if (isset($_GET['exec'], $_POST['token']) && !FM_READONLY) {
    if(!verifyToken($_POST['token'])) {
        fm_set_msg("Invalid Token.", 'error');
    }
...
if (isset($_POST['unzip'], $_POST['token']) && !FM_READONLY) {
    ...
    $unzip = str_replace('/', '', fm_clean_path($unzip));
    $isValid = false;

    $path = os_path_join(FM_ROOT_PATH, FM_PATH);

    $zip_path = os_path_join($path, $unzip);
    if ($unzip != '' && is_file($zip_path)) {
        $ext = pathinfo($zip_path, PATHINFO_EXTENSION);
        $isValid = true;
    } else {
        audit_action('unpack', "path=$zip_path,msg=file-not-found", 'ok');
        fm_set_msg(lng('File not found'), 'error');
    }

    if (($ext == "zip" && !class_exists('ZipArchive')) || ($ext == "tar" && !class_exists('PharData'))) {
        audit_action('unpack', "path=$zip_path,msg=operations-with-archives-not-available", 'error');
        fm_set_msg(lng('Operations with archives are not available'), 'error');
        $FM_PATH=FM_PATH; fm_redirect(FM_SELF_URL . '?p=' . urlencode($FM_PATH));
    }
    ...
    if ($isValid) {
        ...
        if (isset($_POST['tofolder'])) {
            ...
            if (fm_mkdir(os_path_join($path, $tofolder), true)) {
                $path .= '/' . $tofolder;
            }
        }
...
if (isset($_POST['chmod'], $_POST['token']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    if ($file == '' || (!is_file($file_path) && !is_dir($file_path))) {
        audit_action('chmod', "path=$file_path,msg=file-not-found", 'error');
        fm_set_msg(lng('File not found'), 'error');
        $FM_PATH=FM_PATH; fm_redirect(FM_SELF_URL . '?p=' . urlencode($FM_PATH));
    }
...
if (isset($_POST['chown'], $_POST['token']) && !FM_READONLY && !FM_IS_WIN && posix_getuid()==0) {
    if(!verifyToken($_POST['token'])) {
        fm_set_msg(lng("Invalid Token."), 'error');
    }
    $path = os_path_join(FM_ROOT_PATH, FM_PATH);

    $file = str_replace('/', '', fm_clean_path($_POST['chown']));
    $file_path = os_path_join($path, $file);
    if ($file == '' || (!is_file($file_path) && !is_dir($file_path))) {
...
if (is_array($objects) && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        $new_path = os_path_join($path, $file);
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    ?>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/dropzone/<?php echo DROPZONE_VERSION ?>/min/dropzone.min.css" integrity="<?php echo DROPZONE_INTEGRITY_CSS ?>" crossorigin="anonymous" rel="stylesheet">
    <div class="path">
        ...
        <div class="card mb-2 fm-upload-wrapper <?php echo fm_get_theme(); ?>">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    ...
                    <li class="nav-item">
                        <a class="nav-link js-url-upload" href="#urlUploader" data-target="#urlUploader"><i class="fa fa-link"></i> <?php echo lng('Upload from URL') ?></a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                <p class="card-text">
                    ...
                    <strong><?php echo lng('DestinationFolder') ?></strong>: <?php echo fm_enc(fm_convert_win(os_path_join(FM_ROOT_PATH, FM_PATH))) ?>
                </p>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/dropzone/<?php echo DROPZONE_VERSION ?>/min/dropzone.min.js" integrity="<?php echo DROPZONE_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
    <script>
        ...
        <div class="card <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p class="break-word"><strong><?php echo lng('SourceFolder') ?></strong>: <?php echo fm_enc(fm_convert_win(os_path_join(FM_ROOT_PATH, FM_PATH))) ?><br>
                        <label for="inp_copy_to"><strong><?php echo lng('DestinationFolder') ?></strong>:</label>
                        <?php echo os_path_join(FM_ROOT_PATH, '/') ?><input type="text" name="copy_to" id="inp_copy_to" value="<?php echo fm_enc(FM_PATH) ?>">
                    </p>
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            <strong>Destination folder:</strong> <?php echo fm_enc(fm_convert_win(os_path_join(FM_ROOT_PATH, FM_PATH))) ?>
        </p>
        <p>
        ...
        <ul class="folders break-word">
            ...
            foreach ($folders as $f) {
                ?>
                <li>
                    <a href="?p=<?php echo urlencode(trim(os_path_join(FM_PATH, $f, '/'))) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php echo fm_convert_win($f) ?></a></li>
                <?php
            }
            ?>
        </ul>
        ...
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                   ...
                   <div class="mb-3 row">
                        <label for="js-hide-cols" class="col-sm-3 col-form-label"><?php echo lng('CalculateFolderSize') ?></label>
                        <div class="col-sm-9">
                            <div class="form-check form-switch">
                    ...
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                ...
                                <li class="list-group-item"><a href="https://hub.docker.com/r/jpralvesatdocker/tinyfilemanager" target="_blank"><i class="fa fa-cube"></i> Docker image documentation</a></li>
                                <?php if(!FM_READONLY) { ?>
                                <li class="list-group-item"><a href="javascript:show_new_pwd();"><i class="fa fa-lock"></i> <?php echo lng('Generate new password hash') ?></a></li>
                                <?php } ?>
                                <li class="list-group-item"><a data-bs-toggle="collapse" href="#collapseInfo" role="button" aria-expanded="false" aria-controls="collapseInfo"><i class="fa fa-microchip"></i> System Info</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                   ...
                   <div class="collapse" id="collapseInfo">
                          <div class="card card-body">
                            <ul class="list-group list-group-flush">
                              ...
                              <li class="list-group-item">PHP modules: <b><?php echo implode(', ', get_loaded_extensions()); ?></b></li>
                              <?php if (!FM_IS_WIN): ?>
                              <li class="list-group-item">Running as root: <b><?php echo posix_getuid() == 0 ? 'yes' : 'no'; ?></b></li>
                              <?php endif ?>
                              <li class="list-group-item">Memory usage: <b><?php echo fm_get_filesize(memory_get_usage(true)); ?></b></li>
                              <?php if (!FM_IS_WIN): ?>
                              <li class="list-group-item">Load AVG: <b><?php echo implode(", ", array_map(function($num){return number_format($num,2);}, sys_getloadavg())); ?></b></li>
                              <?php endif ?>
                              ...
                              <li class="list-group-item">ROOT fs: <b><?php echo $root_path; ?></b></li>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <p class="break-word">
                ...
                <strong>Modify Date:</strong> <?php echo $modif ?><br>
            ...
            <div class="d-flex align-items-center mb-3">
                ...
                <?php if ( substr($path, 0, strlen($_SERVER['DOCUMENT_ROOT'])) == $_SERVER['DOCUMENT_ROOT']): ?>
                    <b class="ms-2"><a href="<?php echo fm_enc(substr(FM_PATH, strlen($_SERVER['DOCUMENT_ROOT'])) . '/' . $file) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                <?php endif; ?>
                <?php
                // ZIP actions
...
if (isset($_GET['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    <div class="path">
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <p class="card-text">
                    <strong>Full path:</strong> <?php echo fm_enc($file_path) ?><br>
                </p>
                <form action="" method="post">
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
...
if (isset($_GET['chown']) && !FM_READONLY && !FM_IS_WIN && posix_getuid()==0) {
    $file = str_replace('/', '', fm_clean_path($_GET['chown']));
    $file_path = os_path_join($path, $file);
    if ($file == '' || (!is_file($file_path) && !is_dir($file_path))) {
        fm_set_msg('File not found', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
...
<form action="" method="post" class="pt-3">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
                ...
                <?php if (!FM_READONLY): ?>
                    <th style="width:3%" class="custom-checkbox-header">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="js-select-all-items" onclick="checkbox_toggle()" aria-label="Select All">
            ...
            $ii = 3399;
            foreach ($folders as $f) {
                $file_path = os_path_join($path, $f);
                $is_link = is_link($file_path);
                $img = $is_link ? 'icon-link_folder' : 'fa fa-folder-o';
                $modif_raw = filemtime($file_path);
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                if ($calc_folder) {
                    $filesize_raw = fm_get_directorysize($file_path);
                    $filesize = fm_get_filesize($filesize_raw);
                ...
                <tr>
                    <?php if (!FM_READONLY): ?>
                        ...
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="<?php echo $ii ?>" name="file[]" value="<?php echo fm_enc($f) ?>" aria-label="<?php echo fm_enc($f) ?>">
                            <label class="custom-control-label" for="<?php echo $ii ?>"></label>
                        </div>
                        </td><?php endif; ?>
                    <td data-sort=<?php echo fm_convert_win(fm_enc($f)) ?>>
                        <div class="filename"><a href="?p=<?php echo urlencode(trim(os_path_join(FM_PATH, $f, '/'))) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win(fm_enc($f)) ?>
                            </a><?php echo($is_link ? ' &rarr; <i>' . readlink($file_path) . '</i>' : '') ?></div>
                    </td>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo lng('ChangePermissions') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td><?php if (!FM_READONLY && posix_getuid()==0): ?><a title="<?php echo lng('ChangeOwnership') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chown=<?php echo urlencode($f) ?>"><?php endif; ?>
                          <?php echo $owner . ':' . $group ?><?php if (!FM_READONLY && posix_getuid()==0): ?></a><?php endif; ?></td>
                    <?php endif; ?>
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('CopyTo')?>..." href="?p=&amp;copy=<?php echo urlencode(trim(os_path_join(FM_PATH, $f, '/'))) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
                        <?php if ( substr($path, 0, strlen($_SERVER['DOCUMENT_ROOT'])) == $_SERVER['DOCUMENT_ROOT']): ?>
                            <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(substr(FM_PATH, strlen($_SERVER['DOCUMENT_ROOT'])) . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php
            ...
            foreach ($files as $f) {
                $file_path = os_path_join($path, $f);
                $is_link = is_link($file_path);
                $img = $is_link ? 'fa fa-file-text-o' : fm_get_file_icon_class($file_path);
                $modif_raw = filemtime($file_path);
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                $filesize_raw = fm_get_size($file_path);
                $filesize = fm_get_filesize($filesize_raw);
                ...
                <tr>
                    <?php if (!FM_READONLY): ?>
                        <td class="custom-checkbox-td">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="<?php echo $ik ?>" name="file[]" value="<?php echo fm_enc($f) ?>" aria-label="<?php echo fm_enc($f) ?>">
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        <div class="filename">
                        <?php
                           if (( substr($path, 0, strlen($_SERVER['DOCUMENT_ROOT'])) == $_SERVER['DOCUMENT_ROOT']) && (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif')))): ?>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        ...
                        <td><?php if (!FM_READONLY && posix_getuid()==0): ?><a title="<?php echo lng('ChangeOwnership') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chown=<?php echo urlencode($f) ?>"><?php endif; ?>
                        <?php echo fm_enc($owner . ':' . $group) ?><?php if (!FM_READONLY && posix_getuid()==0): ?></a><?php endif; ?></td>
                    <?php endif; ?>
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('CopyTo') ?>..."
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(os_path_join(FM_PATH, $f, '/'))) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <?php if ( substr($path, 0, strlen($_SERVER['DOCUMENT_ROOT'])) == $_SERVER['DOCUMENT_ROOT']): ?>
                            <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(substr(FM_PATH, strlen($_SERVER['DOCUMENT_ROOT'])) . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('Download') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, '1212','<?php echo lng('Download').' '.lng('File'); ?>','<?php echo urlencode($f) ?>', this.href);"><i class="fa fa-download"></i></a>
                        <?php if ( $is_exec && !FM_READONLY ): ?>
                        <a title="<?php echo lng('Execute') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;exec=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, '1213','<?php echo lng('Execute').' '.lng('File'); ?>','<?php echo urlencode($f) ?>', this.href);"><i class="fa fa-bolt"></i></a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php
                ...
                <tfoot>
                    <tr>
                        <td class="gray" colspan="<?php echo (!FM_IS_WIN && !$hide_Cols) ? (FM_READONLY ? '6' :'7') : (FM_READONLY ? '4' : '5') ?>">
                            <?php echo lng('FullSize').': <span class="badge text-bg-'.FM_THEME.' border-radius-0">'.fm_get_filesize($all_files_size).'</span>' ?>
                            <?php echo lng('File').': <span class="badge text-bg-'.FM_THEME.' border-radius-0">'.$num_files.'</span>' ?>
                            <?php echo lng('Folder').': <span class="badge text-bg-'.FM_THEME.' border-radius-0">'.$num_folders.'</span>' ?>
                            <?php echo lng('PartitionSize').': <span class="badge text-bg-'.FM_THEME.' border-radius-0">'.fm_get_filesize(@disk_free_space($path)) .'</span>'; ?>
                            <?php echo lng('FreeOf').' <span class="badge text-bg-'.FM_THEME.' border-radius-0">'.fm_get_filesize(@disk_total_space($path)).'</span>'; ?>
                            <?php $disk_percent = intval((@disk_total_space($path)-@disk_free_space($path))*100/(1+@disk_total_space($path)));
                                  $percent_color = array('success','info','warning','danger')[intval($disk_percent/25)];
                            ?>
                            <div style="display: inline-block">
                            <div class="progress bg-<?php echo FM_THEME ?>" style="width: 100px; border-radius: 5px;border-style: solid; border-width: 1px; !important">
                              <div class="progress-bar bg-<?php echo $percent_color ?> text-<?php echo FM_THEME == 'light' ? 'dark' : 'light' ?>" role="progressbar" style="width: <?php echo $disk_percent ?>%;" aria-valuenow="<?php echo $disk_percent ?>" aria-valuemin="0" aria-valuemax="100" aria-label="disk free percent"><?php echo $disk_percent ?>%</div>
                              </div>
                            </div>
                        </td>
                    </tr>
...
function fm_rdelete($path)
{
    if (is_link($path)) {
        ...
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    if (!fm_rdelete(os_path_join($path, $file))) {
                        $ok = false;
                    }
                }
            }
...
function fm_rchmod($path, $filemode, $dirmode)
{
    if (is_dir($path)) {
        ...
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    if (!fm_rchmod(os_path_join($path, $file), $filemode, $dirmode)) {
                        return false;
                    }
                }
            }
...
function fm_rcopy($path, $dest, $upd = true, $force = true)
{
    if (is_dir($path)) {
        ...
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    if (!fm_rcopy(os_path_join($path, $file), os_path_join($dest, $file))) {
                        $ok = false;
                    }
                }
            }
    ...
    global $stderr;
    fwrite($stderr, "'{$path}' is not a file or a folder." . PHP_EOL);
    return false;
}
    ...
    if ($directory !== false && $directory != '' && file_exists($directory)){
        try {
          foreach(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($directory, FilesystemIterator::SKIP_DOTS)) as $file){
            ...
            fwrite($stderr, "unable to read from folder $directory" . PHP_EOL);
        }
    }
    return $bytes;
}
 ...
 */
 function scan($dir = '', $filter = '') {
    $path = os_path_join(FM_ROOT_PATH, $dir);
     if($dir or $dir == "") {
         ...
         foreach ($rii as $file) {
             if (!$file->isDir()) {
                 ...
                 if (FM_ROOT_PATH != '/') {
                     ...
                     $location = substr($file->getPath(), 1);
                 }
                 $files[] = array(
                     "name" => $fileName,
                     "type" => "file",
        ...
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    $file_path = os_path_join($path, $file);
                    if (is_dir($file_path)) {
                        if (!$this->addDir($file_path)) {
                            return false;
                        }
                    } elseif (is_file($file_path)) {
                        if (!$this->zip->addFile($file_path)) {
                            return false;
                        }
                    }
                }
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a class="navbar-brand" href="?p=" aria-label="Homepage"> <?php echo lng('AppTitle') ?> </a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='?p=' aria-label='Homepage'><i class='fa fa-home' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@<?php echo BOOTSTRAP_VERSION ?>/dist/css/bootstrap.min.css" integrity="<?php echo BOOTSTRAP_INTEGRITY_CSS ?>" crossorigin="anonymous">
    <style>
...
<script src="https://code.jquery.com/jquery-<?php echo JQUERY_VERSION ?>.min.js" integrity="<?php echo JQUERY_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@<?php echo BOOTSTRAP_VERSION ?>/dist/js/bootstrap.bundle.min.js" integrity="<?php echo BOOTSTRAP_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
</body>
</html>
<?php
}
...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@<?php echo BOOTSTRAP_VERSION ?>/dist/css/bootstrap.min.css" integrity="<?php echo BOOTSTRAP_INTEGRITY_CSS ?>" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/<?php echo FONT_AWESOME_VERSION ?>/css/font-awesome.min.css" integrity="<?php echo FONT_AWESOME_INTEGRITY_CSS ?>" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/ekko-lightbox/<?php echo EKKO_LIGHTBOX_VERSION ?>/ekko-lightbox.css" integrity="<?php echo EKKO_LIGHTBOX_INTEGRITY_CSS ?>" crossorigin="anonymous" />
    <?php if (FM_USE_HIGHLIGHTJS && isset($_GET['view'])): ?>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/<?php echo HIGHLIGHT_VERSION ?>/styles/<?php echo FM_HIGHLIGHTJS_STYLE ?>.min.css" integrity="<?php echo HIGHLIGHT_INTEGRITY_CSS ?>" crossorigin="anonymous">
    <?php endif; ?>
    ...
    if (FM_THEME == "dark"): ?>
        <style>
            ...
            pre, theme-dark .navbar-nav i, .navbar-nav .dropdown-toggle, .break-word { color: #CFD8DC; }
...
<script src="https://code.jquery.com/jquery-<?php echo JQUERY_VERSION ?>.min.js" integrity="<?php echo JQUERY_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@<?php echo BOOTSTRAP_VERSION ?>/dist/js/bootstrap.bundle.min.js" integrity="<?php echo BOOTSTRAP_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
<script src="https://cdn.datatables.net/<?php echo DATATABLES_VERSION ?>/js/jquery.dataTables.min.js" integrity="<?php echo DATATABLES_INTEGRITY_JS ?>" crossorigin="anonymous" defer></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ekko-lightbox/<?php echo EKKO_LIGHTBOX_VERSION ?>/ekko-lightbox.min.js" integrity="<?php echo EKKO_LIGHTBOX_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
<?php if (FM_USE_HIGHLIGHTJS && isset($_GET['view'])): ?>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/<?php echo HIGHLIGHT_VERSION ?>/highlight.min.js" integrity="<?php echo HIGHLIGHT_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
...
<script>
    ...
    function save_settings($this) {
        ...
        $.ajax({
            ...
            success: function (data) {if(data) { /*window.location.reload();*/ window.location.href="?p=<?php echo urlencode(FM_PATH) ?>";}}
        }); return false;
    }
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE && !FM_READONLY):
        ...
        $ext =  $ext == "js" ? "javascript" :  ($ext == "" ? "text" : $ext);
        ?>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/ace/<?php echo ACE_VERSION ?>/ace.min.js" integrity="<?php echo ACE_INTEGRITY_JS ?>" crossorigin="anonymous"></script>
    <script>
...
function send_remote_syslog($message, $message_string, $type, $program = "tinyfilemanager") {
  ...
  global $syslog_server,  $syslog_port, $syslog_proto, $use_syslog, $syslog_json, $syslog_facility, $syslog_hostname, $stderr, $syslog_sock;
  ...
  if ($use_syslog) {
    ...
    if ($syslog_json) {
      $m = json_encode(array_change_key_case($message, CASE_LOWER), JSON_UNESCAPED_SLASHES);
    } else {
      $m = $message_string;
    }
    ...
    if ($sock) {
      try {
        ...
        // socket_close($sock); Do not close socket.
      } catch(Exception $e) {
...
function get_file_owner($file_path) {
    ...
    if (function_exists('posix_getpwuid')) {
        if (empty($ownerlist) || !array_key_exists($uid, $ownerlist)) {
    ...
    return $ownerlist[$uid];
}

function get_file_group($file_path) {
    global  $grouplist;
    $gid = filegroup($file_path);
    if (function_exists('posix_getgrgid')) {
        if (empty($grouplist) || !array_key_exists($gid, $grouplist)) {
...
function getusernames_etcpasswd() {
    global $ownerlist;
    ...
    if (empty($ownerlist)) {
        ...
        if($handle) {
            while ( ($values = fgetcsv($handle, 1000, ':')) !== false ) {
                ...
                $ownerlist[(string)$result['uid']] = $result['name'];
            }
            fclose($handle);
        }
    }
...
function getgroupnames_etcgroup() {
    global $grouplist;

    if (empty($grouplist)) {
        $keys = ['name', 'passwd', 'gid', 'grouplist'];
        $handle = fopen('/etc/group', 'r');
        if($handle) {
            while ( ($values = fgetcsv($handle, 1000, ':')) !== false ) {
                $result = array_combine($keys, $values);
                $grouplist[(string)$result['gid']] = $result['name'];
            }
            fclose($handle);
        }
    }
